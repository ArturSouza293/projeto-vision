          ...
          <div>
            ...
            {profile ? (
              <div className="mt-1 flex items-baseline gap-2">
                <span className="font-heading text-2xl font-semibold text-primary">{t(`riskProfile.${profile}`)}</span>
                <span className="text-sm text-muted-foreground">
                  · {t("suitability.score")} <span className="font-medium text-foreground tabular-nums">{score}</span>/100
                </span>
              </div>
            ) : (
              <p className="mt-1 text-sm text-muted-foreground">{t("suitability.answerAll")}</p>
            )}
          </div>
          <Button disabled={busy || !allAnswered} onClick={runSuitability} variant={profile ? "outline" : "default"}>
            <Sparkles className="size-4" />
            {profile ? t("suitability.recompute") : t("suitability.compute")}
          </Button>
        </div>
      ...
      <div className="space-y-3">
        {SUITABILITY_QUESTIONS.map((q, i) => (
          <section key={q.id} className="rounded-2xl border border-border bg-card p-5">
            ...
            <RadioGroup
              value={suitability.answers[q.id] !== undefined ? String(suitability.answers[q.id]) : undefined}
